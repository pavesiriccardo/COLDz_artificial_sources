...
fwhm=2.355
import sys,numpy as np,pyfits,os
...
inp=open('reduint.dat','r')
reduint=cPickle.load(inp)
inp.close()
reduint=[red for red in reduint if not np.isnan(red[0])]
...
def func1():
	signal=pyfits.open('signal_smooth.fits')
	outp=open('aper_fluxes_smooth.txt','w')
	for idx,obj in enumerate(reduint):
		coord=obj[1]
		bin=obj[2][1]
		spat_size=obj[2][0]
		cube_narrow=signal[0].section[max([0,(coord[2]-bin/2)]):min([2015,(coord[2]+bin/2)]),coord[1]-30:coord[1]+30,coord[0]-30:coord[0]+30]#signal[0].section[(coord[2]-bin/2):(coord[2]+bin/2),coord[1]-30:coord[1]+30,coord[0]-30:coord[0]+30]
		...
		try:
			...
			approx_cent_line=coord[2]*2*chanwidth+29.9605
			flux_factor=beam_factor(coord[0],coord[1],approx_cent_line,[fits[2015*(pnt-1)+coord[2],5] for pnt in range(1,58)],rr[coord[2]])		
			...
			cent_line=(fitspec[0]+startchan)*2*chanwidth+29.9605#coord[2]*2*chanwidth+29.9605
...
def func2():
	signal=pyfits.open('signal_smooth.fits')
	outp=open('1pix_fluxes_smooth.txt','w')
	for idx,obj in enumerate(reduint):
		coord=obj[1]
		chanwidth=0.002
		bin=obj[2][1]
		spat_size=obj[2][0]
		cube_narrow=signal[0].section[max([0,(coord[2]-bin/2)]):min([2015,(coord[2]+bin/2)]),coord[1]-30:coord[1]+30,coord[0]-30:coord[0]+30]#cube_narrow=signal[0].section[(coord[2]-bin/2):(coord[2]+bin/2),coord[1]-30:coord[1]+30,coord[0]-30:coord[0]+30]
		startchan=max([0,coord[2]-25])
		endchan=min([2015,coord[2]+25])
		cube=signal[0].section[startchan:endchan,coord[1]-30:coord[1]+30,coord[0]-30:coord[0]+30] 
		#ys, xs = np.mgrid[:2*spat_size, :2*spat_size]
		#y,x=np.mgrid[:60,:60]
		img=np.sum(cube_narrow,0)
		small_size=max(2*spat_size,8)
		img2=img[30-small_size/2:30+small_size/2,30-small_size/2:30+small_size/2]
		peak=[np.argmax(img2)/small_size,np.argmax(img2)%small_size]
		try:
			spectrum=[cube[ix][peak[0]+30-small_size/2,30-small_size/2+peak[1]] for ix in range(cube.shape[0])]
			fitspec,covm_spec=curve_fit(gauss,range(cube.shape[0]),spectrum,(coord[2]-startchan,bin/fwhm,2e-3))
			cent_line=(fitspec[0]+startchan)*2*chanwidth+29.9605
			cent_line_err=(np.sqrt(covm_spec[0,0]))*2*chanwidth
			Sdeltv=fitspec[2]*np.sqrt(2*np.pi)*fitspec[1]*2*chanwidth/cent_line*3e5
...
#################################
#measure 1-pix flux in the no_smooth mosaic
def func3():
	signal_nosmooth=pyfits.open('signal_nosmooth.fits')
	outp=open('1pix_fluxes_nosmooth.txt','w')
	for idx,obj in enumerate(reduint):
		coord=obj[1]
		chanwidth=0.002
		bin=obj[2][1]
		spat_size=obj[2][0]
		cube_narrow=signal_nosmooth[0].section[max([0,(coord[2]-bin/2)]):min([2015,(coord[2]+bin/2)]),coord[1]-30:coord[1]+30,coord[0]-30:coord[0]+30]#cube_narrow=signal_nosmooth[0].section[(coord[2]-bin/2):(coord[2]+bin/2),coord[1]-30:coord[1]+30,coord[0]-30:coord[0]+30]
		startchan=max([0,coord[2]-25])
		endchan=min([2015,coord[2]+25])
		cube=signal_nosmooth[0].section[startchan:endchan,coord[1]-30:coord[1]+30,coord[0]-30:coord[0]+30] 
		img=np.sum(cube_narrow,0)
		small_size=max(2*spat_size,8)
		img2=img[30-small_size/2:30+small_size/2,30-small_size/2:30+small_size/2]
		peak=[np.argmax(img2)/small_size,np.argmax(img2)%small_size]
